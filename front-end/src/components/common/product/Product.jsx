import React from 'react'

import { LuShoppingCart } from "react-icons/lu";


const Product = () => {

    const products = [
        {id:1, img:"./src/assets/products/product (1).jpg", title:"Cà phê chồn", price:30, description:"Cà phê chồn"},
        {id:2, img:"./src/assets/products/product (2).jpg", title:"Cà phê sữa đá", price:35, description:"Cà phê sữa đá"},
        {id:3, img:"./src/assets/products/product (3).jpg", title:"Cà phê đen đá", price:15, description:"Cà phê đen đá"},
        {id:4, img:"./src/assets/products/product (1).jpg", title:"Cà phê chồn", price:30, description:"Cà phê chồn"},
        {id:5, img:"./src/assets/products/product (2).jpg", title:"Cà phê sữa đá", price:35, description:"Cà phê sữa đá"},
        {id:6, img:"./src/assets/products/product (3).jpg", title:"Cà phê đen đá", price:15, description:"Cà phê đen đá"},
    ]
  return (
      <div className='flex justify-center mt-10 mb-10'>
        
        <div className='w-[1024px] flex flex-col items-center'>
        <div className='flex flex-wrap justify-center  '>

            {products.map(({id, img, title, price, description}) => (
                <div key={id} className='group text-[#27272f] mx-5 my-5 relative'>
                    <div className=' rounded-lg overflow-hidden bg-[#beb996]'>
                        <img className='group-hover:scale-105 group-hover:opacity-50 group-hover:rotate-1 transition-all duration-700 ease-in-out' src={img} alt="" />
                    </div>
                    <div className=''>
                        <div className=' flex px-3 group-hover:text-[#de9190] transition-all duration-700 ease-in-out ' >
                            <h2 className='regular-20 tracking-wider capitalize my-3 mr-auto'><a className='no-underline  ' href="/">{title}</a></h2>
                            <LuShoppingCart  className='regular-20 tracking-wider capitalize my-auto ' />
                        </div>
                        <div className=' flex text-[#777777] regular-16 tracking-wider px-3'>
                            <p className='mr-auto'>{description}</p> 
                            <p>{price}k</p> 
                        </div>
                    </div>
                </div>  
            ))}
        </div>
      </div>
      </div>
    )
}

export default Product